...
### Change Brand & ASCII Colors

Edit `src/config/theme.js`. `quasar.config.js` reads `APP_THEME.brand` for Quasar's brand colors, while the studio store hydrates its default ASCII colors and appearance presets from `APP_THEME.ascii`:
```javascript
export const APP_THEME = {
  ...
  ascii: {
    defaultTheme: { fg: '#41ff6e', bg: '#040b06' },
    themes: [
      { labelKey: 'themes.brand', fg: '#ff694d', bg: '#05071a' }
    ]
  }
}
```
